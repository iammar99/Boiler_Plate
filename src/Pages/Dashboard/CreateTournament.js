import React, { useEffect, useState } from 'react'
import { message } from 'antd'
// --------------------------- Images ---------------------------
import table from "../../Assets/table.jpg"
import poll from "../../Assets/polls.png"
// --------------------------- firebase ---------------------------
import { setDoc, doc, getDocs, collection, query, where } from 'firebase/firestore'
import { firestore } from 'Config/firebase'
// --------------------------- Navigation ---------------------------
import { useNavigate } from 'react-router-dom'
// --------------------------- Responsive table ---------------------------
import { Table, Thead, Tbody, Tr, Th, Td } from 'react-super-responsive-table'

export default function CreateTournament() {




    // --------------------------- Mode ---------------------------
    const [mode, setMode] = useState("")

    // --------------------------- Team Count ---------------------------
    const [team, setTeam] = useState()

    // --------------------------- Team Names ---------------------------
    const [teamNames, setTeamNames] = useState([]);

    // // --------------------------- Overs ---------------------------
    // const [overs, setOvers] = useState("");

    // --------------------------- Tournament name ---------------------------
    const [name, setName] = useState("");



    // --------------------------- Function to select mode ---------------------------
    const handleMode = (type) => {
        setMode(type)
        console.log("mode", type)
    }


    const navigate = useNavigate()


    // --------------------------- Function to select names ---------------------------
    const handleTeamNameChange = (index, value) => {
        const newTeamNames = [...teamNames];
        let obj = {
            name: value,
            rank: index + 1,
            win: 0,
            lose: 0,
            runRate: 0.00,
        }
        newTeamNames[index] = obj;
        setTeamNames(newTeamNames);
    };

    // --------------------------- Function to create matches ---------------------------

    const generateMatches = (teams) => {
        const matches = [];
        for (let i = 0; i < teams.length; i++) {
            for (let j = i + 1; j < teams.length; j++) {
                matches.push({
                    teamA: teams[i].name,
                    teamB: teams[j].name,
                    matchId: `${teams[i].name}_vs_${teams[j].name}`.replace(/\s+/g, '_'),
                    result:"TBD",
                    stage:"Group Stage"
                });
            }
        }
        return matches;
    };



    // --------------------------- Function to create tournament ---------------------------
    const handleCreate = async (e) => {
        e.preventDefault()
        let user = JSON.parse(localStorage.getItem("User"))
        const randomId = Math.floor(1000000 + Math.random() * 9000000).toString();
        if (teamNames.length != team) {
            message.error("Please enter all team names")
            return
        }
        for (let i = 0; i < teamNames.length; i++) {
            if (teamNames[i].name.length < 2) {
                message.error("Please enter valid team name of team number " + (i + 1))
                return
            }
        }
        const matches = await generateMatches(teamNames)
        let tournament = {
            tournamentName: name,
            id: randomId,
            mode: mode,
            team: teamNames,
            createrName: user.username,
            createrId: user.id,
            status: "live",
            matches: matches,
            // overs:overs
        }
        try {
            await setDoc(doc(firestore, "tournaments", randomId), tournament);
            message.success("Tournament Successfully Created");
            navigate("/dashboard/tournament")

        } catch (error) {
            console.log(error)
        }

        // try {
        //     // 1. Generate a proper random ID (ensured to be string)
        //     const randomId = Math.floor(1000000 + Math.random() * 9000000).toString();

        //     // 2. Validate tournament data before saving
        //     if (!tournament || typeof tournament !== 'object') {
        //         throw new Error('Invalid tournament data');
        //     }

        //     // 3. Create document reference with validation
        //     const docRef = doc(firestore, "tournaments", randomId);

        //     // 4. Set the document
        //     await setDoc(docRef, tournament);

        //     console.log("Tournament created with ID:", randomId);
        //     return randomId;// Return the ID for future reference
        // } catch (error) {
        //     console.error("Error creating tournament:", error);
        //     throw error; 
        // }
    }



    return (
        <main>
            <h1 className="text-center my-4">
                <>
                    {/*?xml version="1.0" encoding="utf-8"?*/}
                    <svg
                        version="1.1"
                        id="Layer_1"
                        xmlns="http://www.w3.org/2000/svg"
                        xmlnsXlink="http://www.w3.org/1999/xlink"
                        x="0px"
                        y="0px"
                        viewBox="0 0 110.26 122.88"
                        style={{ enableBackground: "new 0 0 110.26 122.88", "width": "35px", "marginRight": "10px" }}
                        xmlSpace="preserve"
                    >
                        <style
                            type="text/css"
                            dangerouslySetInnerHTML={{
                                __html: ".st0{fill-rule:evenodd;clip-rule:evenodd;}"
                            }}
                        />
                        <g>
                            <path
                                className="st0"
                                d="M89.43,45.54c3.38-1,6.39-2.61,8.84-4.65c4.02-3.35,6.5-7.9,6.5-12.87c0-2.78-0.77-5.42-2.15-7.78l0.01,0 c-0.62-1.06-1.36-2.07-2.21-3.01c-0.44,6.2-1.96,12.08-4.34,17.43C94.32,38.61,92.07,42.27,89.43,45.54L89.43,45.54L89.43,45.54z M56.4,91.21l2.83,7.93l8.42,0.24l-6.67,5.14l2.37,8.08l-6.95-4.76l-6.95,4.76l2.37-8.08l-6.67-5.14l8.42-0.24L56.4,91.21 L56.4,91.21z M20.63,80.94h71.55v41.94H20.63V80.94L20.63,80.94z M60.73,60.07v6.55h0.49v4.58l-12.42,0v-4.58l0.49,0v-6.59 c-7.57-1.17-14.49-4.59-20.14-9.59l-0.67,1.85c-7.79-0.13-14.84-2.82-19.95-7.08C3.26,40.83,0,34.75,0,28.01 c0-3.8,1.05-7.4,2.93-10.63l-0.01-0.01c1.9-3.27,4.66-6.13,8.03-8.37L14,6.97l-3.53,0V0h88.61v6.97h-2.82L99.31,9 c3.36,2.24,6.11,5.1,8.02,8.37h0.01c1.87,3.22,2.92,6.82,2.92,10.64c0,6.74-3.26,12.81-8.54,17.21 c-5.11,4.26-12.16,6.95-19.95,7.08l-0.67-1.86C75.39,55.5,68.39,58.94,60.73,60.07L60.73,60.07L60.73,60.07z M42.16,75.79h25.71 v4.58H42.16V75.79L42.16,75.79z M46.08,71.2h17.87v4.58H46.08V71.2L46.08,71.2z M11.99,40.89c2.45,2.05,5.47,3.65,8.85,4.66 c-2.64-3.27-4.89-6.93-6.66-10.9c-2.38-5.34-3.9-11.23-4.34-17.43c-0.83,0.92-1.57,1.94-2.2,3.01l-0.01,0 c-1.37,2.35-2.14,5-2.14,7.78C5.49,32.99,7.97,37.53,11.99,40.89L11.99,40.89z"
                            />
                        </g>
                    </svg>
                </>

                Create Tournament
            </h1>
            <p className="text-center my-">
                Let's create a tournament together. Please fill in the required fields below.
            </p>
            {/* <form className="create-form"> */}

            {/* -------------------  First Select Mode ------------------- */}

            {
                mode.length > 0
                    ?
                    <>
                        {
                            mode == "Table"
                                ?
                                <>
                                    <section className="table">
                                        <div className="container-fluid">
                                            <div className="row">
                                                <div className="col">
                                                    <form className="table-form">



                                                        {/* ------------- Name of teams ------------- */}

                                                        <div className="mb-3">
                                                            <label htmlFor="name" className="form-label">
                                                                Enter the name of the tournament
                                                            </label>
                                                            <input
                                                                onChange={(e) => { setName(e.target.value) }}
                                                                type="text"
                                                                className="form-control"
                                                                id="name"
                                                                value={name} />
                                                        </div>

                                                        {/* ------------- Number of Overs ------------- */}
{/* 
                                                        <div className="mb-3">
                                                            <label htmlFor="overs" className="form-label">
                                                                Enter the number of overs
                                                            </label>
                                                            <input
                                                                onChange={(e) => { setOvers(e.target.value) }}
                                                                type="number"
                                                                className="form-control"
                                                                id="overs"
                                                                value={overs} />
                                                        </div> */}

                                                        {/* ------------- Number of teams ------------- */}

                                                        <div className="mb-3">
                                                            <label htmlFor="count" className="form-label">
                                                                Enter the number of teams
                                                            </label>
                                                            <input
                                                                onChange={(e) => { setTeam(parseInt(e.target.value)) }}
                                                                type="number"
                                                                className="form-control"
                                                                id="count"
                                                                value={team} />
                                                        </div>


                                                        {
                                                            team > 0
                                                                ?
                                                                <>

                                                                    {/* ------------- Names of teams ------------- */}

                                                                    <div className="mb-3">
                                                                        <label htmlFor="Name" className="form-label fs-4">
                                                                            Enter the Names of teams :
                                                                        </label>

                                                                        {Array.from({ length: team }).map((_, index) => (
                                                                            <div key={index} className="mb-3">
                                                                                <label htmlFor={`team-${index}`} className="form-label">
                                                                                    Team {index + 1} Name:
                                                                                </label>
                                                                                <input
                                                                                    type="text"
                                                                                    className="form-control"
                                                                                    id={`team-${index}`}
                                                                                    // value={JSON.stringify(teamNames[index]) || ''}
                                                                                    onChange={(e) => handleTeamNameChange(index, e.target.value)}
                                                                                    required
                                                                                />
                                                                            </div>
                                                                        ))}
                                                                    </div>

                                                                    {/* ------------- Names of teams ------------- */}
                                                                    <button className="pushable" onClick={handleCreate}>
                                                                        <span className="shadow" />
                                                                        <span className="edge" />
                                                                        <span className="front"> Push Me </span>
                                                                    </button>
                                                                </>
                                                                :
                                                                <></>
                                                        }


                                                    </form>
                                                </div>
                                            </div>
                                        </div>
                                    </section>
                                </>
                                :
                                <>
                                    <section className="poll">
                                        <div className="container-fluid">
                                            <div className="row">
                                                <div className="col">
                                                    Poll Mode Seleted
                                                    {mode}
                                                </div>
                                            </div>
                                        </div>
                                    </section>
                                </>
                        }
                    </>
                    :

                    <section className="mode">
                        <div className="container-fluid">
                            <div className="row">
                                <div className="col-12 col-md-6">
                                    <div className="card mx-auto my-4 d-flex justify-content-center points-table" style={{ "padding": "20px", "width": "fit-content" }} onClick={() => { handleMode("Table") }}>
                                        <img style={{ "height": "300px", "width": "300px", "margin": "auto" }} src={table} alt="" />
                                    </div>
                                    <h5 className="text-center my-2">Table Mode</h5>
                                </div>
                                <div className="col-12 col-md-6">
                                    <div className="card mx-auto my-4 d-flex justify-content-center polls" style={{ "padding": "20px", "width": "fit-content" }} onClick={() => { handleMode("Poll") }} >
                                        <img style={{ "height": "300px", "width": "300px", "margin": "auto" }} src={poll} alt="" />
                                    </div>
                                    <h5 className="text-center my-2">Poll Mode</h5>
                                </div>
                            </div>
                        </div>
                    </section>
            }

            {/* </form> */}
        </main>
    )
}
